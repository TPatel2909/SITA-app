...
export class AppUtil {
	...
	constructor(){
		AppUtil.isMobile = AppUtil._isMobile();
	}
	...
	public handleArray(mainObj:any,valObj:any,arrayName:any){
		if(valObj[arrayName] == null || (valObj[arrayName] != null && typeof(valObj[arrayName]) === "string")){
			...
			let tempAry:any = valObj[arrayName];
	...
	public setFieldValue(mainObj:any,valObj:any){
		let fields = Object.keys(valObj);
		for(let eachFld of fields){
	...
	public setFieldValueByType(mainObj:any,valObj:any){
		let fields = Object.keys(mainObj);
		for(let eachFld of fields){
			if(eachFld.startsWith("@") == false){
	...
	getTotalByProperty(prop:any,ary:any,format:any){
		...
		let formatted = (format != null) ? numeral(total).format(format) : total;
		let nonFormatted = numeral(formatted).value();
		
		return {formatted:formatted,nonFormatted:nonFormatted};
	}
	...
	public getRouteConfigFromURL(rt:any){
		let rtConfig = this.getRouteConfig(rt);
	...
	getRouteConfig(rt:any){
		let rts = [];
		let curRt = decodeURIComponent(rt);
		let allPrms = curRt.split(";");
		let prms:any = {};
		let count = 0;
		for(let each of allPrms){
			if(count == 0)
	...
	public static isNull(aryRitm:any,isAny:any){
		...
		if(Array.prototype.isPrototypeOf(aryRitm)){
			...
			let trs = isItNullAry.filter((item)=>item==true);
			if(isAny == true){
				if(trs.length > 0)
					return true;
				else
	...
	public static getPremForPOI(premium:any,startDate:any,endDate:any){
		let diffDays = moment(endDate,"YYYY-MM-DD").diff(moment(startDate,"YYYY-MM-DD"), 'days') + 1;
		let endDateMoment=moment(endDate,"YYYY-MM-DD");
        let effDateMoment=moment(startDate,"YYYY-MM-DD");
        let dateRange = moment.range(effDateMoment, endDateMoment);
		...
		*/
        let output = this.dateRangeContainsLeapYear(dateRange);
	...
	public static getCookieByName(cname:any) {
		let name = cname + "=";
	...
	public static decFormatNoRound(val:any,format:any){
		let mult = parseInt("1"+format.split(".")[1]);
	...
	public static getNotInList(aryItems:any,path:any){
		...
		if(AppUtil.isNull(aryItems,false) == false && aryItems.length > 0){
			let newArr:any = [];
			for(let item of aryItems)
			{
				let itmVal = AppUtil.getValueByPath(item,path);
